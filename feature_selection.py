...
def tests(model_function, forwards, num_features, num_tests):
    ...
    while i <= num_tests:
        ...
        features, acc  = get_important_features(sfs_selector.subsets_,sfs_selector.subsets_[index]['avg_score'], sfs_selector.subsets_[index]['feature_idx'])
...
def perform_tests(forwards, num_features, num_tests):
    ...
    print("-------------------------------- SVM --------------------------------")
    now = timer()
    svm_freqs, svm_accuracy, svm_old_accuracy, svm_avg_num_features = tests(svm.SVC(C=100), forwards= forwards, num_features= num_features, num_tests= num_tests)
    time = timer() - now
    
    svm_results["svm_freqs"] = svm_freqs
    svm_results["svm_accuracy"] = svm_accuracy
    svm_results["svm_old_accuracy"] = svm_old_accuracy
    svm_results["svm_avg_num_features"] = svm_avg_num_features
    svm_results["svm_cum_time"] = time
    
    print("-------------------------------- RF --------------------------------")
    now= timer()
    rf_freqs, rf_accuracy, rf_old_accuracy, rf_avg_num_features = tests(RandomForestClassifier(n_estimators=5), forwards=forwards, num_features =num_features,num_tests= num_tests)
    time = timer() - now
    
    rf_results["rf_freqs"] = rf_freqs
    rf_results["rf_accuracy"] = rf_accuracy
    rf_results["rf_old_accuracy"] = rf_old_accuracy
    rf_results["rf_avg_num_features"] = rf_avg_num_features
    rf_results["rf_cum_time"] = time
    
    print("-------------------------------- KNN --------------------------------")
    now= timer()
    knn_freqs, knn_accuracy, knn_old_accuracy, knn_avg_num_features = tests(KNeighborsClassifier(n_neighbors=8), forwards= forwards, num_features= num_features, num_tests= num_tests)
    time = timer() - now

    knn_results["knn_freqs"] = knn_freqs
    knn_results["knn_accuracy"] = knn_accuracy
    knn_results["knn_old_accuracy"] = knn_old_accuracy
    knn_results["knn_avg_num_features"] = knn_avg_num_features
    knn_results["knn_cum_time"] = time
    
    # Write to json files
    filenames = ["svm_SFS.json", "rf_SFS.json", "knn_SFS.json"]
    if(not forwards):
        filenames=["svm_SBS.json", "rf_SBS.json", "knn_SBS.json"]
