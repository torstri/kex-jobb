...
\alttitle{Värdering av attribut för klassificering av hudcancer med hjälp av attributselektion}
...
The frequency of skin cancer has in the last decade been on the rise in Sweden with approximately 60 000 people being diagnosed with the disease yearly. \parencite{sverige-hudcancer}.
...
Malignant melanoma is the most dangerous form of skin cancer and in 2021 around 4800 people in Sweden were diagnosed with the disease(Cancerfonden https://www.cancerfonden.se/om-cancer/statistik/malignt-melanom-i-huden). Although dangerous, the 5-year survival rate is about of 99\% if discovered at an early stage \parencite{skincancer_org_2019}.
...
K-nearest-neighbors is used for classifying data points. The ground principle is to find the nearest neighboring data points from the training set for all the test data points. This is based on the idea that similar data points can be found close to one another. When classifying, the model regards the $k$ the nearest data points and classifies the test data point based on which class the majority of these neighbors have. \parencite{ibmknn}

The algorithm is simple to implement and does not have many hyperparameters to tune. However, one drawback is that it suffers from the curse of dimensionality, meaning that the model does not perform well with many features. \parencite{ibmknn}
...
Neural networks are a type of machine learning algorithm that is inspired by the human brain. The network consists of several layers of nodes, where each node is connected to all nodes in the previous layer. The first layer is the input layer, where one node corresponds to one feature from the input data. The last layer is the output layer, where the final prediction is made. The layers in between are called hidden layers. All nodes in one layer is connected to all nodes in the next layer. Each connection between two nodes has a weight, which is adjusted during the training of the model. The weights are adjusted in order to minimize the loss function, which is a measure of how well the model performs. \parencite{ibmWhatNeural}

When classifying, the input data is fed into the input layer. The data is then passed through the network, where each node in the hidden layers calculates a weighted sum of the inputs and applies an activation function to the result.
The output of the neurons in the last hidden layer is then fed into the output layer, where the final prediction is made. \parencite{ibmWhatNeural}
...
Random forest is a tree-based method based on the idea of bagging. Bagging is a method where several models are trained on different subsets of the training data. This is done in order to reduce the correlation between different trees. The final prediction is made by a majority vote between the individual models. \parencite{ibmrforest}
...
% Kan skriva om att det kan ge overfitting om jag minns rätt men då behövs källa
...
\begin{table}[h!]
  \caption{Average number of occurrences in optimal set for Morphological and SIFT features.}
  \pgfplotstabletypeset[ col sep=comma,
    ...
    columns/Occurence(s)/.style={column name=Occurrences, column type={l|}},
    columns/Representation/.style={column name=Average Occurrences, column type={l|}},
...
% Latex-magi
\begin{table}[h!]
  \caption{Average number of occurrences in optimal set for Morphological and SIFT features grouped by classifier.}
  \pgfplotstabletypeset[ col sep=comma,
    string type,
    ...
    columns/Morphological Occurences/.style={column name=Morphological Occurrences, column type={>{\raggedright\arraybackslash}p{2.5cm}|}},
    columns/Occurences per Morphological Feature/.style={column name=Occurrences per Morphological Feature, column type={>{\raggedright\arraybackslash}p{2.5cm}|}},
    columns/SIFT Occurences/.style={column name=SIFT Occurrences, column type={>{\raggedright\arraybackslash}p{2.5cm}|}},
    columns/Occurences per SIFT Feature/.style={column name=Occurrences per SIFT Feature, column type={>{\raggedright\arraybackslash}p{2.5cm}|}},
    every head row/.style={before row=\hline, after row=\hline},
    every nth row={1}{before row=\hline},
    every last row/.style={after row=\hline}
...
In the following subsections the results for every classifier is presented through both a figure showing the distribution of feature occurrences and a table giving more details into which features were the most preferred.
...
Similarly to the neural network, the difference between the two methods is not vast in terms of performance. The SBS outperformed the SFS method, utilizing fewer features for a greater gain in accuracy. Secondly, like the two aforementioned classifiers, there is a variance between the different tests for both selection methods.

Lastly, in the case of the SFS method, there are spikes in all the tests, where the performance is greatly increased when adding just a few features. 
...
Two features were of significant importance compared to all other and both of these features were morphological. The most prominent was a feature describing the compactness of the lesion and the second most important feature described the contrast between the lesion and the surrounding skin in terms of the color red.
...
With these points for future research we hope that the subject area can further improve to help tackle the problem of accurately classifying skin cancer in an effective manner, making it more available to patients. 
